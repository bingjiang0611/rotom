...
> 2026-09-17 用户授权的提交身份修正及旧/新 SHA 映射见 [history-identity-repair.md](history-identity-repair.md)；未增加邮箱例外或放宽扫描规则。
...
Pi CLI/SDK 闭包随包携带后，实测 tgz 约 51 MiB，单个当前产物递归扫描约 263 MiB；公开历史保留多个已审查发行版后，完整 tree+history 扫描约 463 MiB。因此归档输入上限为 64 MiB、累计扫描为 512 MiB；普通文件/归档成员仍为 32 MiB，嵌套深度仍有界，超限依然 BLOCKED，不跳过旧版本、依赖或二次归档。
