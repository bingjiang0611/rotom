...
`snapshot_visible` 源码增加可选 `text`（节点 name 的大小写不敏感子串）与 `role`（大小写敏感精确角色）筛选，两者为 AND。例如 `browser_inspect({operation:"snapshot_visible", tabName:"form", text:"Save", role:"button"})`。每次首次查询仍从 Relay 获取新快照，在模型输出层过滤，不执行页面 JS、不更改原始 ref/generation/epoch；不带筛选时行为不变。返回 `selection` 标记查询条件与 `sourceNodes`，`availableNodes` 是匹配数；超过字节预算或显式 limit 时复用既有分页，后续只传 cursor/limit，不重复筛选。查询仅覆盖经过既有 sanitizer 的有界快照（最多 1,000 节点、每个 name 最多 1,024 UTF-8 bytes），不是 DOM 全局搜索；保留来源 truncated，contentComplete 始终 false，零匹配不证明页面不存在目标。需要全文仍走 `snapshot` / `read_full`；若实际任务证明目标常被来源边界裁掉，再评估 Relay 端定向查询，不放宽完成标志。该变更只经确定性 L1 验证，未测真实 Chrome L2/L3 或 provider token 收益；需安装更新后的源码并启动新 session，不要求 Chrome 重载，也未覆盖活跃安装。
