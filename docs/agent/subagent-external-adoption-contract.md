...
> **历史设计与采用合同**：下文“当前安装 `.2` / 仅 opt-in”描述第十二批结束时的状态。后续已内化 `pi-subagents@0.52.1-rotom.1`，并从 `alpha.3` 起由 launcher 默认启用 scoped 执行，见 [当前默认与限制](subagent-owned-default.md)。本页保留采用过程、授权范围与证据，不作为当前安装操作指南。
