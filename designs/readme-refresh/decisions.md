...
- Reference / reuse：读取 beautify 的视觉、hero、SVG、GitHub canvas 与内容组织指南；从现行产品合同取材，无外部素材。设计库查询无匹配 README 案例；旧 `P-PI-MODEL-SELECTOR` 来源已因用户清理原 Pi design 目录失效，不把缺失记录当成可复用证据。
...
- Library receipt：已注册 `P-ROTOM-README`，权威来源为本文件；共享索引 `~/.agents/skills/design-loop/library/index.html`，查询 `rotom README` 命中 available。全库 check 仍因旧 `P-PI-MODEL-SELECTOR` 来源被用户删除而失败，本次不恢复该记录、不宣称全库通过。
...
- Preserve：保留原 SVG 封面，不重画素材；详细说明及已提交的启动界面说明完整迁移；不删除组件、评测和上游各自的 README。
...
- Source：采用 Pokémon Bank / normal 的 Heat Rotom，姿态与用户参考一致；未修改原始字节，来源和权利说明在 `assets/readme/ATTRIBUTION.md`。它不属于当时的仓库 MIT 授权，不进入 npm 产品运行时；当前许可状态以 `docs/usage.md` 为准。
