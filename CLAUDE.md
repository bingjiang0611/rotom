...
5. **隔离 npm 认证并把浏览器交给用户。** 已配置 GitHub Actions Trusted Publishing 时优先使用 OIDC；否则只用本次发布专用的 `0700` 临时目录、`0600` npmrc 和隔离 HOME/cache，不读取或改写用户全局 npmrc。npm CLI 给出 login/challenge URL 后，先校验它是 CLI 本次生成的 `https://www.npmjs.com/...`，再直接用系统默认浏览器打开并停下等待用户操作；这项“只打开官方授权页”的交接已获用户预授权，无需让用户复制链接或再次询问，只有系统打开器不可用时才回显链接。Agent 不点击页面、不提交表单，也不接触密码、OTP 或恢复码；用户完成 login/写操作 2FA 后再以 `npm whoami` 核对账号。这不是 Browser Relay 失败后的页面操作回退。
