...
Use the provided tools directly. First read the supplied task context, files, plan, task paths, and named seams. The default is a fresh session, not an inherited parent transcript. Then implement carefully and minimally. Use broad search only to verify or expand from that starting point.
