...
import { resolveExecutionStore } from "./execution-store.ts";
...
export interface ControlEvent {
	/** Stable across delivery channels and replay; not a supervisor request ID. */
	id?: string;
...
export type ProcessTerminalReason =
	...
	| "external-descendants-unverified"
...
interface ProcessTerminalBaseV1 {
	/** Separate versioned scheduling evidence; never changes the v1 graph state. */
	ownedClosure?: import("../runs/background/owned-execution.ts").OwnedClosureV2;
	version: 1;
	runId: string;
	childIndex?: number;
...
export interface ExternalProcessStatus {
	processInstanceId?: string;
	/** Scope is the creation-time owned group, never arbitrary descendants. */
	processGroup?: ProcessTreeTerminalV1;
	directCloseObserved?: boolean;
...
export interface AsyncStatus {
	ownedExecutionScope?: "owned-process-groups-v2";
	lifecycleArtifactVersion?: SubagentLifecycleArtifactVersion;
	runId: string;
...
export interface WaitSubscriptionRecord {
	/** v1 is read-only migration input; old runtimes reject v2 rather than replay it. */
	version: 1 | 2;
	token: string;
	sessionId: string;
...
export interface RunSyncOptions {
	/** Internal scoped workflow foreground owner, never a caller-provided routing flag. */
	ownedForeground?: import("../runs/background/owned-foreground.ts").OwnedForegroundHandle;
...
export interface ExtensionConfig {
	...
	/** Explicit new-run opt-in; legacy capacity owners are never migrated. */
	asyncExecutionScope?: "owned-process-groups-v2";
...
const legacyTempRoot = configuredTempRoot
	? path.resolve(configuredTempRoot)
	: path.join(os.tmpdir(), `pi-subagents-${resolveTempScopeId()}`);
export const EXECUTION_STORE = resolveExecutionStore(legacyTempRoot);
export const BASE_TEMP_ROOT_DIR = EXECUTION_STORE.baseRoot;
export const TEMP_ROOT_DIR = EXECUTION_STORE.root;
