...
import { OWNER_LIFELINE_ENV } from "./owner-lifeline.ts";
import { STARTUP_TOKEN_ENV } from "./startup-transport.ts";
...
export interface BuildPiArgsInput {
	...
	/** RPC caller owns one-shot task delivery after verified startup. */
	deferTask?: boolean;
...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	} else if (!input.deferTask) {
		args.push(`Task: ${input.task}`);
	}

	const env: Record<string, string | undefined> = {};
	// Each foreground spawn establishes its own descriptor. A detached runner
	// must never inherit an ancestor's descriptor number as proof of ownership.
	if (OWNER_LIFELINE_ENV in process.env) env[OWNER_LIFELINE_ENV] = undefined;
	if (STARTUP_TOKEN_ENV in process.env) env[STARTUP_TOKEN_ENV] = undefined;
	const piPackageRoot =
