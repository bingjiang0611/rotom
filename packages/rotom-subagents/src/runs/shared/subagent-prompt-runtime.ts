...
import { SUBAGENT_CHILD_ENV, SUBAGENT_CHILD_AGENT_ENV, SUBAGENT_CHILD_INDEX_ENV, SUBAGENT_FANOUT_CHILD_ENV, SUBAGENT_STEER_ACK_DIR_ENV, SUBAGENT_STEER_CAPABILITY_ENV, SUBAGENT_STEER_INBOX_ENV } from "./pi-args.ts";
...
import { registerOwnerLifeline } from "./owner-lifeline.ts";
import { STARTUP_TOKEN_ENV, STARTUP_TOKEN, STARTUP_STATUS_PREFIX, STARTUP_READY } from "./startup-transport.ts";
...
export default function registerSubagentPromptRuntime(pi: ExtensionAPI): void {
	const owner = registerOwnerLifeline(pi, { required: process.platform !== "win32" && process.env[SUBAGENT_CHILD_ENV] === "1" });
	...
	const startupToken = process.env[STARTUP_TOKEN_ENV];
	if (startupToken !== undefined) {
		pi.on("session_start", (_event, ctx) => {
			if (!STARTUP_TOKEN.test(startupToken) || ctx.mode !== "rpc" || !owner.isReady()) {
				ctx.abort(); ctx.shutdown(); return;
			}
			ctx.ui.setStatus(STARTUP_STATUS_PREFIX + startupToken, STARTUP_READY);
		});
		// Use Pi's settled/idle/shutdown contract, not a replicated print loop.
