...
import { OWNED_EXECUTION_SCOPE, type OwnedExecutionRequest, type OwnedWorkflowParent } from "./owned-execution.ts";
...
interface AsyncExecutionContext {
	ownedExecutionScope?: typeof OWNED_EXECUTION_SCOPE;
...
function launchOwnedExecution(ctx: AsyncExecutionContext, capacity?: ActiveAsyncCapacityHandle): OwnedExecutionRequest | undefined {
	assertExecutionStoreScope(EXECUTION_STORE, ctx.ownedExecutionScope);
	if (capacity && (capacity.owner.scope !== ctx.ownedExecutionScope || capacity.owner.storeId !== EXECUTION_STORE.storeId)) throw new Error("Owned launch capacity/store identity mismatch.");
	if (ctx.ownedExecutionScope !== OWNED_EXECUTION_SCOPE) return undefined;
	return { scope: OWNED_EXECUTION_SCOPE, storeId: EXECUTION_STORE.storeId!, ...(ctx.ownedWorkflowParent ? { workflowParent: ctx.ownedWorkflowParent } : {}), ...(capacity ? { capacity: { ownerSessionId: capacity.owner.ownerSessionId, reservationToken: capacity.owner.reservationToken, generation: capacity.owner.generation } } : {}) };
}
...
export function executeAsyncChain(
	...
	try {
		assertExecutionStoreScope(EXECUTION_STORE, params.ctx.ownedExecutionScope);
		if (EXECUTION_STORE.scope) {
			params = { ...params, sessionRoot: params.sessionRoot ?? scopedSessionRoot(EXECUTION_STORE)! };
			assertScopedSessionPath(EXECUTION_STORE, params.sessionRoot!);
			for (const file of params.sessionFilesByFlatIndex ?? []) if (file) assertScopedSessionPath(EXECUTION_STORE, file);
		}
	}
	catch (error) { return formatAsyncStartError("chain", error instanceof Error ? error.message : "Execution store unavailable."); }
	const {
		chain,
		agents,
		ctx,
		...
		spawnResult = spawnRunner(
			{
				...
				ownedExecution: launchOwnedExecution(ctx, params.activeAsyncCapacity),
				asyncDir,
...
export function executeAsyncSingle(
	...
	try {
		...
		if (EXECUTION_STORE.scope) {
			params = { ...params, sessionRoot: params.sessionRoot ?? scopedSessionRoot(EXECUTION_STORE)! };
			for (const file of [params.sessionRoot, params.sessionDir, params.sessionFile, params.revivalLease?.sessionFile]) if (file) assertScopedSessionPath(EXECUTION_STORE, file);
		}
	}
	catch (error) { return formatAsyncStartError("single", error instanceof Error ? error.message : "Execution store unavailable."); }
	const {
		agent,
		agentConfig,
		ctx,
