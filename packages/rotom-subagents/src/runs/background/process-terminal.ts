...
import { validOwnedExecution, projectOwnedClosure, type OwnedExecutionV2 } from "./owned-execution.ts";
...
export interface ProcessTerminalCandidate {
	...
	externalWritersUnverified?: boolean;
	ownedExecution?: OwnedExecutionV2;
	sessionFile?: string;
...
/** Fixed persisted fields only; never infer writer closure from result prose.
 * Status, final results and terminal candidates must retain the same fence even
 * when another lifecycle artifact or the active index is absent.
 */
export function hasUnverifiedExternalWriters(value: unknown): boolean {
	if (!isRecord(value)) return false;
	const external = (entry: unknown): boolean => isRecord(entry) && (
		(isRecord(entry.runner) && entry.runner.type === "external-cli")
		|| (isRecord(entry.externalProcess) && entry.externalProcess.descendantScope === "unverified")
	);
	return value.externalWritersUnverified === true || value.reason === "external-descendants-unverified"
		|| external(value) || (Array.isArray(value.steps) && value.steps.some(external))
		|| (Array.isArray(value.results) && value.results.some(external));
}
...
export function readProcessTerminalCandidate(asyncDir: string): ProcessTerminalCandidate | undefined {
	try {
		...
		if (raw.externalWritersUnverified !== undefined && typeof raw.externalWritersUnverified !== "boolean") throw new Error("Invalid external writer evidence.");
		if (raw.ownedExecution !== undefined && (!validOwnedExecution(raw.ownedExecution) || raw.ownedExecution.runId !== raw.runId || raw.ownedExecution.runnerProcessInstanceId !== raw.runnerProcessInstanceId)) throw new Error("Invalid owned execution evidence.");
		...
		return {
			...
			...(raw.externalWritersUnverified === true ? { externalWritersUnverified: true } : {}),
			...(validOwnedExecution(raw.ownedExecution) ? { ownedExecution: raw.ownedExecution } : {}),
	...
	try {
		...
		if (status.steps) {
			for (const [index, step] of status.steps.entries()) {
				...
				// expectedWriters counts Pi writers only. Zero cannot prove an external
				// command never started; its owned group is not the whole writer graph.
				const external = step.runner?.type === "external-cli";
				const externalUnknown = proof.state === "unknown" && proof.reason === "external-descendants-unverified";
				const stepState = external ? "unknown" : expected === 0 ? "not-started" : proof.state === "observed" && records.length === expected ? "observed" : proof.state === "pending" ? "pending" : "unknown";
				step.processTerminal = stepProcessTerminalProof(proof, index, stepState, records, externalUnknown ? "unavailable" : resumeDisposition(step.status, step.sessionFile ?? candidate?.sessionFile));
			}
		}
...
export function finalizeProcessTerminal(
	...
	try {
		...
		if (!candidate) proof = unknownProof(runId, runnerClose.processInstanceId, "runner-candidate-missing");
			...
			if (candidate.externalWritersUnverified || status?.steps?.some((step) => step.runner?.type === "external-cli")) {
				// External commands can escape via setsid. Owned-group cleanup cannot
				// authorize a whole-writer-graph terminal or release its active index.
				proof = unknownProof(runId, runnerClose.processInstanceId, "external-descendants-unverified");
