...
import { hasUnverifiedExternalWriters } from "./process-terminal.ts";
import { hasOwnedExecutionEvidence } from "./owned-execution.ts";
...
function runSkipReason(input: {
	...
	const evidence = [status, readJson(path.join(runDir, "process-terminal.json")), readJson(path.join(runDir, "process-terminal-candidate.json"))];
	if (evidence.some(hasOwnedExecutionEvidence)) return "owned-execution-evidence";
	if (evidence.some(hasUnverifiedExternalWriters)) return "external-descendants-unverified";
...
function resultSkipReason(input: {
	...
	if (hasOwnedExecutionEvidence(input.data)) return "owned-execution-evidence";
	if (hasUnverifiedExternalWriters(input.data)) return "external-descendants-unverified";
