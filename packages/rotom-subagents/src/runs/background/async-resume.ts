...
import { EXECUTION_STORE, DIRS, type AcceptanceInput, type AsyncStatus, type ResolvedTurnBudget, type SteeringRecoveryDescriptor, type SubagentRunMode } from "../../shared/types.ts";
...
import { hasOwnedExecutionEvidence, ownedClosureObserved } from "./owned-execution.ts";
import { readOwnedForeground } from "./owned-foreground.ts";
...
export function resolveAsyncResumeTarget(params: AsyncResumeParams, deps: AsyncResumeDeps = {}, options: AsyncResumeOptions = {}): AsyncResumeTarget {
	...
	// Fence before reconciliation, including result-only fallback and mixed runs.
	// Read fixed structured fields only: a task result is not closure evidence.
	let externalUnverified = false;
	let scoped = Boolean(EXECUTION_STORE.scope), ownedClosed = false;
	if (location.asyncDir) {
		if (readOwnedForeground(location.asyncDir)) throw new Error("Owned foreground workflow child recovery is unavailable; do not bypass its parent fence.");
		if (readOwnedWorkflow(location.asyncDir)) throw new Error("Owned workflow recovery is unavailable; capacity release does not authorize replay.");
		let raw: AsyncStatus | undefined;
		try { raw = JSON.parse(fs.readFileSync(path.join(location.asyncDir, "status.json"), "utf-8")) as AsyncStatus; }
		catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
		const proof = readProcessTerminal(location.asyncDir);
		const candidate = readProcessTerminalCandidate(location.asyncDir);
		if (proof?.ownedClosure?.workflowParent || candidate?.ownedExecution?.workflowParent) throw new Error("Owned workflow child recovery is unavailable; do not bypass its parent fence.");
		externalUnverified = [proof, candidate, raw].some(hasUnverifiedExternalWriters);
		scoped ||= [proof, candidate, raw].some(hasOwnedExecutionEvidence);
		ownedClosed = ownedClosureObserved(proof?.ownedClosure, {
			runId: location.resolvedId ?? path.basename(location.asyncDir),
			runnerProcessInstanceId: raw?.processTerminal?.runnerProcessInstanceId ?? candidate?.runnerProcessInstanceId ?? proof?.runnerProcessInstanceId ?? "",
		});
	}
	if (location.resultPath) {
		const raw = ensureObject(JSON.parse(fs.readFileSync(location.resultPath, "utf-8")), location.resultPath);
		externalUnverified ||= hasUnverifiedExternalWriters(raw);
		scoped ||= hasOwnedExecutionEvidence(raw);
	}
	if (externalUnverified) throw new Error("External execution unknown: descendant closure is unverified. Inspect the original run; do not retry or replace its writer.");
	if (scoped && !ownedClosed) throw new Error("Owned execution closure unavailable: do not resume, retry or replace its writer.");
