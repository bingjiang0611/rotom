...
import { EXECUTION_STORE, type TokenUsage } from "../../shared/types.ts";
import { scopedSessionRoot, assertScopedSessionPath } from "../../shared/execution-store.ts";
import {
	cleanupWorktrees,
	createWorktrees,
	diffWorktrees,
...
import { acquireSessionLease, inspectSessionLease, canonicalSessionId, type SessionLeaseRequest } from "../shared/session-lease.ts";
...
interface SubagentRunConfig {
	...
	ownedExecution?: OwnedExecutionRequest;
...
function runPiStreaming(
	...
	return new Promise((resolve) => {
		...
		let ownedLease: ReturnType<typeof acquireSessionLease> | undefined;
		if (childEventContext?.ownedExecutionRequired) {
			const dir = path.dirname(outputFile), sessionFile = childEventContext.sessionFile;
			if (childEventContext.sessionEnabled === false) bindOwnedWriterLease(dir, processInstanceId, { state: "not-held", reason: "session-disabled" });
			else {
				if (!sessionFile) throw new Error("Scoped Pi writer requires a creation-time session destination.");
				assertScopedSessionPath(EXECUTION_STORE, sessionFile);
				if (!onWriterProcess) {
					fs.mkdirSync(path.dirname(sessionFile), { recursive: true, mode: 0o700 });
					try { fs.closeSync(fs.openSync(sessionFile, "wx", 0o600)); }
					catch (error) { if ((error as NodeJS.ErrnoException).code !== "EEXIST" || !ownedSessionPreviouslyReleased(dir, childEventContext.runId, sessionFile)) throw new Error("Scoped fresh writer cannot import an existing session file."); }
					ownedLease = acquireSessionLease({ sessionFile, runId: childEventContext.runId, sourceRunId: childEventContext.runId });
					onWriterProcess = writer => ownedLease!.updateWriter(writer);
				}
				const current = inspectSessionLease(sessionFile);
				if (current.state !== "owned" || current.owner.version !== 2 || current.owner.storeId !== EXECUTION_STORE.storeId || current.owner.runId !== childEventContext.runId) throw new Error("Scoped Pi writer lease identity unavailable.");
				bindOwnedWriterLease(dir, processInstanceId, { state: "held", sessionFile: current.owner.canonicalSessionFile, canonicalSessionId: canonicalSessionId(sessionFile), token: current.owner.token });
			}
		}
		onWriterProcess?.({ state: "spawning" });
		const outputStream = fs.createWriteStream(outputFile, { flags: "w" });
		const spawnEnv = { ...process.env, ...(env ?? {}), ...getSubagentDepthEnv(maxSubagentDepth) };
		// buildPiArgs removes inherited descriptor authority; this spawn creates
		// a fresh pipe owned by this runner rather than disabling the child guard.
		if (process.platform !== "win32") spawnEnv[OWNER_LIFELINE_ENV] = "3";
		...
		let processCloseSeen = false, forcedOwnedStdio = false;
		let ownedDrainTimer: NodeJS.Timeout | undefined;
		let groupCompletion: Promise<ProcessTreeTerminalV1> | undefined;
		const finishGroup = (): Promise<ProcessTreeTerminalV1> => groupCompletion ??= (processTreeController
			? processTreeController.finishAfterWriterClose()
			: Promise.resolve({ state: "unknown" as const, reason: "verification-failed" as const, diagnostic: "Writer PID was unavailable." }))
			.catch(() => ({ state: "unknown" as const, reason: "verification-failed" as const, diagnostic: "Group close unavailable." }));
		...
		const clearStdioGuard = childEventContext?.ownedExecutionRequired
			? () => { if (ownedDrainTimer) clearTimeout(ownedDrainTimer); }
			: attachPostExitStdioGuard(child, { idleMs: 2000, hardMs: 8000 });
		// Both streams are explicit pipes; the four-fd spawn overload loses that type refinement.
		...
		child.on("exit", () => {
			childExited = true;
			clearDrainTimers();
			if (childEventContext?.ownedExecutionRequired) void finishGroup().then(() => {
				if (processCloseSeen) return;
				ownedDrainTimer = setTimeout(() => { forcedOwnedStdio = true; child.stdout?.destroy(); child.stderr?.destroy(); }, 1000);
				ownedDrainTimer.unref?.();
			});
		});
...
async function runSingleStepInner(
	...
	if (ctx.ownedExecutionRequired && sessionEnabled && !step.sessionFile) step.sessionFile = path.join(ctx.sessionDir ?? scopedSessionRoot(EXECUTION_STORE)!, ctx.id, `step-${ctx.flatIndex}.jsonl`);
	...
	if (step.runner?.type === "external-cli") {
		...
		const external = await runExternalCli(omitUndefinedProperties({
			ownedExecutionRequired: ctx.ownedExecutionRequired,
	...
	modelAttemptsLoop: while (modelIndex < candidates.length) {
		...
		const run = await runPiStreaming(
			...
			{ eventsPath, runId: ctx.id, stepIndex: ctx.flatIndex, agent: step.agent, ownedExecutionRequired: ctx.ownedExecutionRequired === true, sessionEnabled, sessionFile: step.sessionFile },
...
async function runSubagent(
	...
	if (config.ownedExecution) {
		if (!config.runnerProcessInstanceId) throw new Error("Scoped runner identity missing.");
		beginOwnedExecution(asyncDir, id, config.runnerProcessInstanceId, config.ownedExecution);
	}
	...
	const statusPayload: RunnerStatusPayload = omitUndefinedProperties({
		...(config.ownedExecution ? { ownedExecutionScope: config.ownedExecution.scope } : {}),
		...
		if (isDynamicRunnerGroup(step)) {
			...
			const parallelResults = await mapConcurrent(dynamicSteps, concurrency, async (task, taskIdx): Promise<StepResult> => {
				...
				const singleResult = await runSingleStepWithTimeout(task, compactOptional<SingleStepContext>({
					...
					ownedExecutionRequired: config.ownedExecution !== undefined,
	...
	if (config.runnerProcessInstanceId) {
		...
		// Imported roots, external jobs and nested writer graphs need their own
		// roster barriers. Do not release based on only this runner's local rows.
		const coverageComplete = !flatSteps.some((step) => step.importAsyncRoot || step.runner?.type === "external-job")
			&& !statusPayload.steps.some((step) => (step.children?.length ?? 0) > 0);
		const ownedExecution = sealOwnedExecution(asyncDir, id, config.runnerProcessInstanceId, coverageComplete);
		const candidate: ProcessTerminalCandidate = {
			...
			writers,
			expectedWriters,
			externalWritersUnverified: statusPayload.steps.some((step) => step.runner?.type === "external-cli"),
...
async function runConfiguredSubagent(config: SubagentRunConfig): Promise<void> {
	...
	const releaseOnExit = (): void => {
		try {
			if (!config.ownedExecution || ownedExecutionCanReleaseLease(config.asyncDir, config.id)) lease?.release();
		} catch {
			// Scoped leases remain opaque/occupied after owner loss; exit is not closure.
		}
	};
	...
	try {
		...
		if (lease) {
			...
			try {
				if (!config.ownedExecution || ownedExecutionCanReleaseLease(config.asyncDir, config.id)) acknowledged = lease.release();
			} catch (error) {
				console.error("Failed to release session revival lease:", error);
			}
			try {
				if (config.ownedExecution) markOwnedWriterLeaseRelease(config.asyncDir, lease.owner.token, acknowledged);
