...
import { assertExecutionStoreScope, scopedSessionRoot, assertScopedSessionPath } from "../../shared/execution-store.ts";
import { beginOwnedForeground, returnForegroundHost, assertForegroundRecoveryAllowed, type OwnedForegroundHandle } from "../background/owned-foreground.ts";
import { beginOwnedWorkflow, registerWorkflowAdmission, bindWorkflowAdmission, closeOwnedWorkflow, type OwnedWorkflow } from "../background/owned-workflow.ts";
import type { OwnedWorkflowParent } from "../background/owned-execution.ts";
...
interface ExecutionContextData {
	ownedForeground?: OwnedForegroundHandle;
	ownedWorkflowParent?: OwnedWorkflowParent;
...
function executionSessionRoot(deps: ExecutorDeps, parent: string | null): string {
	return scopedSessionRoot(EXECUTION_STORE) ?? deps.getSubagentSessionRoot(parent);
}
function trustedSessionRootsForStatus(ctx: ExtensionContext, deps: ExecutorDeps): string[] {
	const scoped = scopedSessionRoot(EXECUTION_STORE);
...
function appendStepToAsyncChain(input: {
	...
	const asyncCtx = compactOptional<Parameters<typeof executeAsyncSingle>[1]["ctx"]>({
		...
		ownedExecutionScope: input.deps.config.asyncExecutionScope,
...
async function resumeAsyncRun(input: {
	...
	try {
		...
		if (resolved?.kind === "nested") {
			...
			const trustedSessionRoots = [
				...
				...(parentSessionFile || EXECUTION_STORE.scope ? [executionSessionRoot(input.deps, parentSessionFile)] : []),
			];
	...
	if (attachChain) {
		...
		try {
			activeAsyncCapacity = topLevelResume ? acquireActiveAsyncCapacity({
				scope: input.deps.config.asyncExecutionScope,
		...
		permissions: input.deps.config.permissions,
			...
			sessionRoot: executionSessionRoot(input.deps, parentSessionFile),
	...
	let activeAsyncCapacity: ActiveAsyncCapacityHandle | undefined;
	try {
		assertScopedSessionPath(EXECUTION_STORE, revivalSessionFile);
		if (recoveryDescriptor?.sessionDir) assertScopedSessionPath(EXECUTION_STORE, recoveryDescriptor.sessionDir);
	...
	const result = executeAsyncSingle(runId, compactOptional<Parameters<typeof executeAsyncSingle>[1]>({
		...
		sessionRoot: executionSessionRoot(input.deps, parentSessionFile ?? revivalSessionFile),
...
function runAsyncPath(data: ExecutionContextData, deps: ExecutorDeps): AgentToolResult<Details> | null {
	...
	const asyncCtx = compactOptional<Parameters<typeof executeAsyncSingle>[1]["ctx"]>({
		...
		ownedExecutionScope: deps.config.asyncExecutionScope,
		ownedWorkflowParent: data.ownedWorkflowParent,
...
async function runSinglePath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	try {
		r = await runSync(ctx.cwd, agents, params.agent!, task, compactOptional<Parameters<typeof runSync>[4]>({
			ownedForeground: data.ownedForeground,
			permissions: deps.config.permissions,
...
const workflowLaunchObservers = new WeakMap<object, (launch: { agent: string; sessionFile?: string; async: boolean; runId?: string }) => OwnedWorkflowParent | void>();
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		try {
			assertExecutionStoreScope(EXECUTION_STORE, deps.config.asyncExecutionScope);
			if (params.sessionDir) assertScopedSessionPath(EXECUTION_STORE, deps.expandTilde(params.sessionDir));
			if (deps.config.defaultSessionDir) assertScopedSessionPath(EXECUTION_STORE, deps.expandTilde(deps.config.defaultSessionDir));
		}
		catch (error) { return buildRequestedModeError(params, error instanceof Error ? error.message : "Execution store unavailable."); }
		const recoveryAction = typeof params.action === "string" ? params.action.trim() : params.action;
		if (recoveryAction === "resume" || recoveryAction === "steer" || typeof params.resume === "string") {
			try { assertForegroundRecoveryAllowed(DIRS.async, { id: params.id ?? (typeof params.resume === "string" ? params.resume : deps.state.lastForegroundControlId ?? undefined), dir: params.dir }, DIRS.results); }
			catch (error) { return buildRequestedModeError(params, error instanceof Error ? error.message : "Foreground ownership unavailable."); }
		}
		const workflowLaunchObserver = workflowLaunchObservers.get(params);
		// Caller-controlled parent IDs are routing labels, never permission to
		// borrow a workflow slot or bypass its creation-time admission roster.
		if (deps.config.asyncExecutionScope && (params.workflowParentRunId !== undefined || params.workflowKey !== undefined) && !workflowLaunchObserver) {
			return buildRequestedModeError(params, "Scoped workflow child requires an in-process admission owner.");
		}
		...
		if (requestParams.workflowScript !== undefined && normalizedAction === undefined) {
			...
			if (workflowRunId && topLevelAsyncWorkflow) {
				...
				try {
					workflowCapacity = acquireActiveAsyncCapacity({
						scope: deps.config.asyncExecutionScope,
			...
			if (workflowRunId) {
				...
				let ownedWorkflow: OwnedWorkflow | undefined;
				try {
					fs.mkdirSync(asyncDir, { recursive: true });
					writeRunFanoutBudgetDescriptor(asyncDir, workflowFanoutBudget);
					fs.mkdirSync(DIRS.results, { recursive: true });
					if (deps.config.asyncExecutionScope) {
						const owner = workflowCapacity?.owner;
						ownedWorkflow = beginOwnedWorkflow(asyncDir, workflowRunId, currentSessionId, owner ? { ownerSessionId: owner.ownerSessionId, reservationToken: owner.reservationToken, generation: owner.generation } : undefined);
					}
				} catch (error) {
					workflowCapacity?.rollback();
				...
				workflowCapacity?.markWorkflowStarted(ownedWorkflow?.controllerInstanceId);
				...
				let status: AsyncStatus = {
					...(deps.config.asyncExecutionScope ? { ownedExecutionScope: deps.config.asyncExecutionScope } : {}),
					runId: workflowRunId,
					toolCallId,
				...
				const runPersistence = createCapacityResilientJsonWriter({
					...
					write: (filePath, payload) => filePath === resultPath
						? writeAsyncResultFile(filePath, { ...payload as Record<string, unknown>, ...(deps.config.asyncExecutionScope ? { ownedExecutionScope: deps.config.asyncExecutionScope } : {}) })
				...
				void Promise.resolve().then(async () => {
					...
					try {
						const workflow = await runWorkflowScript({
							...
							...(ownedWorkflow ? { onControllerClosed: () => closeOwnedWorkflow(asyncDir, ownedWorkflow!) } : {}),
							timeoutMs: timeout,
							...
							launch: async (key, childParams, workflowSignal, admission) => {
								...
								let ownedForeground: OwnedForegroundHandle | undefined;
								...
								const result = await runMissionWorkflowChild(missionBinding, workflowRunId, key, childPhase, () => {
									...
									workflowLaunchObservers.set(childRequest, (launch) => {
										...
										// Foreground keeps its authoritative completion pipeline; no async receipt conversion.
										const binding = ownedWorkflow && ownedAdmission && launch.runId && !childRequest.resume && !childRequest.worktree && !childRequest.gate
											? bindWorkflowAdmission(asyncDir, ownedWorkflow, ownedAdmission, launch.runId, launch.async ? "async" : "foreground") : undefined;
										if (binding?.kind === "foreground") ownedForeground = beginOwnedForeground(path.join(DIRS.async, binding.childRunId), binding);
									...
									}, ctx, preserveActiveSession).finally(() => { if (ownedForeground) returnForegroundHost(ownedForeground); });
								});
							...
							steer: (key, message, options, workflowSignal) => {
								if (ownedWorkflow) throw new Error("Scoped workflow steering/recovery is not supported; do not replace its writer.");
								return steerWorkflowChildByKey({ state: deps.state, workflowRunId, key, message, options, signal: workflowSignal, resolveRunId: () => workflowChildRunIds.get(key) });
							},
						});
		...
		if (effectiveParams.sessionDir) {
			...
			const baseSessionRoot = deps.config.defaultSessionDir
				...
				: executionSessionRoot(deps, parentSessionFile);
			sessionRoot = path.join(baseSessionRoot, runId);
		}
		try {
			assertScopedSessionPath(EXECUTION_STORE, sessionRoot);
		...
		try {
			...
			if (workflowLaunchObserver) {
				...
				const launch = hasSingle
					? { agent: effectiveParams.agent!, sessionFile: childSessionFileForTask(effectiveParams.agent!, 0, effectiveParams.model), async: effectiveAsync, runId: effectiveAsync ? asyncRunId : runId }
					: singleTask
				...
				if (launch) {
					...
					const binding = workflowLaunchObserver(launch);
					if (binding) {
						execData.ownedWorkflowParent = binding;
						if (binding.kind === "foreground") execData.ownedForeground = { dir: path.join(DIRS.async, binding.childRunId), parent: binding };
					}
				}
			}
