...
import { existsSync, unlinkSync, mkdirSync, openSync, closeSync, lstatSync, realpathSync } from "node:fs";
...
import { registerForegroundWriter, closeForegroundWriter, finishForegroundPipeline, markForegroundCoverageUnavailable } from "../background/owned-foreground.ts";
...
import { attachPostExitStdioGuard, trySignalChild as trySignalProcess } from "../../shared/post-exit-stdio-guard.ts";
...
async function runSingleAttempt(
	...
	let ownedSessionFile = options.ownedForeground ? options.sessionFile ?? path.join(options.sessionDir ?? path.join(options.ownedForeground.dir, "sessions"), `${randomUUID()}.jsonl`) : undefined;
	if (ownedSessionFile) {
		mkdirSync(path.dirname(ownedSessionFile), { recursive: true, mode: 0o700 });
		// Pi's public --session path initializes an empty file. Reserve its inode
		// exclusively before canonical lease acquisition; never write a Pi header
		// ourselves or truncate an existing/forked session.
		try { closeSync(openSync(ownedSessionFile, "wx", 0o600)); }
		catch (error) { if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error; }
		const stat = lstatSync(ownedSessionFile);
		if (!stat.isFile() || stat.isSymbolicLink()) throw new Error("Invalid scoped foreground session file.");
		ownedSessionFile = realpathSync.native(ownedSessionFile);
	}
	const { args, env: sharedEnv, tempDir, toolDiagnosticPath, runtimeAcknowledgedExtensionsPath, capabilityAudit } = buildPiArgs({
		...
		sessionEnabled: Boolean(ownedSessionFile) || shared.sessionEnabled,
		sessionDir: options.sessionDir,
		sessionFile: ownedSessionFile ?? options.sessionFile,
		model: modelArg,
	...
	const exitCode = await new Promise<number>((resolve) => {
		...
		const priorCoverageLost = options.ownedForeground?.coverageLost;
		if (options.ownedForeground) options.ownedForeground.coverageLost = true;
		const ownedLease = options.ownedForeground && ownedSessionFile ? acquireSessionLease({ sessionFile: ownedSessionFile, runId: options.ownedForeground.parent.childRunId, sourceRunId: options.ownedForeground.parent.workflowRunId }) : undefined;
		const ownedWriterId = options.ownedForeground && ownedLease ? registerForegroundWriter(options.ownedForeground, ownedLease) : undefined;
		ownedLease?.updateWriter({ state: "spawning" });
		if (options.ownedForeground) options.ownedForeground.coverageLost = priorCoverageLost;
		const proc = spawn(spawnSpec.command, spawnSpec.args, {
			detached: Boolean(ownedWriterId),
			cwd: options.cwd ?? runtimeCwd,
			env: spawnEnv,
		...
		const ownedTree = ownedWriterId && proc.pid ? createOwnedProcessTreeController(proc.pid) : undefined;
		// Install lifecycle callbacks before publishing the running lease below.
		let ownedCleanup: Promise<ProcessTreeTerminalV1> | undefined;
		let ownedCloseObserved = false, forcedOwnedStdio = false;
		let ownedDrainTimer: NodeJS.Timeout | undefined;
		const cleanupOwned = (): Promise<ProcessTreeTerminalV1> => ownedCleanup ??= ownedTree?.terminate() ?? Promise.resolve({ state: "unknown", reason: "verification-failed" });
		const trySignalChild = (child: typeof proc, signal: NodeJS.Signals): boolean => {
			if (!ownedWriterId || (signal === "SIGINT" && !ownedCleanup)) return trySignalProcess(child, signal);
			void cleanupOwned(); return Boolean(ownedTree);
		};
		...
		const processLine = (line: string) => {
			...
			if (evt.type === "tool_execution_start") {
				if (options.ownedForeground && evt.toolName === "subagent") markForegroundCoverageUnavailable(options.ownedForeground);
		...
		const clearStdioGuard = ownedWriterId
			? () => { if (ownedDrainTimer) clearTimeout(ownedDrainTimer); }
			: attachPostExitStdioGuard(proc, { idleMs: 2000, hardMs: 8000 });
		...
		proc.on("exit", () => {
			childExited = true;
			clearFinalDrainTimers();
			if (ownedWriterId) void cleanupOwned().then(() => {
				if (ownedCloseObserved) return;
				// Escaped descendants can retain pipes after group cleanup. Bound the
				// wait, but never turn destroyed stdio into a genuine close proof.
				ownedDrainTimer = setTimeout(() => { forcedOwnedStdio = true; proc.stdout?.destroy(); proc.stderr?.destroy(); }, 1000);
				ownedDrainTimer.unref?.();
			});
		});
		proc.on("close", async (code, signal) => {
			ownedCloseObserved = true;
			// No detach/interrupt can reopen a closed direct writer while group
			// verification and the canonical lease release are still pending.
			processClosed = true;
			const closeObservedAt = Date.now();
			clearStdioGuard();
			if (ownedWriterId && options.ownedForeground) {
				try {
					const group = await cleanupOwned();
					await jsonlWriter.close().catch(() => {});
					if (!forcedOwnedStdio) {
						const released = group.state === "observed" && ownedLease?.release() === true;
						closeForegroundWriter(options.ownedForeground, ownedWriterId, group, closeObservedAt, released);
					}
					if (ownedSessionFile && existsSync(ownedSessionFile)) result.sessionFile = ownedSessionFile;
				} catch { /* Missing/persistence-failed closure retains capacity. */ }
			}
			if (lifecycleFinished) return;
			processClosed = true;
			clearFinalDrainTimers();
		...
		proc.on("error", (error) => {
			if (ownedWriterId) void cleanupOwned();
			if (lifecycleFinished) return;
			processClosed = true;
			clearFinalDrainTimers();
			clearStdioGuard();
		...
		if (ownedLease && proc.pid) {
			try { ownedLease.updateWriter({ state: "running", pid: proc.pid }); }
			catch { void cleanupOwned(); }
		}
...
export async function runSync(
	...
	const ownedForeground = options.ownedForeground ? structuredClone(options.ownedForeground) : undefined;
	let detachedReason: string | undefined;
	...
	void authoritativeCompletion.then((terminalResult) => {
		...
		// This is the authoritative pipeline, not the possibly earlier detach receipt.
		if (ownedForeground) {
			try { finishForegroundPipeline(ownedForeground); } catch { /* Closure remains unproven. */ }
		}
	});
