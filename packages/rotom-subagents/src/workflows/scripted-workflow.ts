...
export interface RunWorkflowScriptOptions {
	/** Scope-only evidence hook: actual worker exit AND all dispatched host calls settled.
	 * It is independent of task outcome; a missing callback never proves closure. */
...
export async function runWorkflowScript(options: RunWorkflowScriptOptions): Promise<WorkflowScriptResult> {
	...
	const workerExit = options.onControllerClosed ? new Promise<void>((resolve) => worker.once("exit", () => resolve())) : undefined;
	const pendingHostCalls = new Set<Promise<unknown>>();
	const emits: unknown[] = [];
	...
	return await new Promise<WorkflowScriptResult>((resolve, reject) => {
		const finish = (outcome: { value: unknown } | { error: Error & { workflowErrorKind?: unknown } }) => {
			...
			if (options.onControllerClosed) {
				// Stop admission now, but do not confuse task completion with closure.
				// A non-cooperating host call leaves this proof pending, not a blocked result.
				void worker.terminate().catch(() => { /* Only the exit event is evidence. */ });
				void Promise.allSettled([workerExit!, ...pendingHostCalls, ...Array.from(launches.values(), (launch) => launch.promise), ...Array.from(steers.values(), (steer) => steer.promise)]).then(() => {
					try { options.onControllerClosed!(); } catch { console.error("Workflow closure evidence unavailable."); }
				});
			}
			void Promise.allSettled([...steers.values()].map(({ promise }) => promise)).then(() => {
				...
				if (!options.onControllerClosed) void worker.terminate();
		...
		worker.on("message", (message: Record<string, unknown>) => {
			if (settled || (finishing && options.onControllerClosed)) return;
			...
			const respond = (promise: Promise<unknown>) => {
				if (options.onControllerClosed) {
					pendingHostCalls.add(promise);
					void promise.then(() => pendingHostCalls.delete(promise), () => pendingHostCalls.delete(promise));
				}
				void promise.then(
					(value) => {
						if (!settled && !(finishing && options.onControllerClosed)) worker.postMessage({ type: "response", callId: message.callId, ok: true, value: omitUndefinedWorkflowValues(value) });
					},
					(error: unknown) => {
						if (!settled && !(finishing && options.onControllerClosed)) worker.postMessage({ type: "response", callId: message.callId, ok: false, error: error instanceof Error ? error.message : String(error), ...(error instanceof Error && (error as { workflowErrorKind?: unknown }).workflowErrorKind === "detached-child" ? { errorKind: "detached-child" } : {}) });
					},
				);
			};
