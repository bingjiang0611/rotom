...
const SubagentWaitParamsSchema = Type.Object({
	until: Type.Optional(Type.Unsafe<"change" | "terminal">({
		type: "string", enum: ["change", "terminal"],
		description: "Default change returns on attention. terminal keeps waiting through advisory attention, but still returns for a pending supervisor reply, abort, or deadline. Blocking only; cannot combine with nonBlocking.",
	})),
	id: Type.Optional(Type.String({
