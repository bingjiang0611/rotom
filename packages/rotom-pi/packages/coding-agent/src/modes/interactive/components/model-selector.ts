...
export class ModelSelectorComponent extends Container implements Focusable {
	...
	private onSelectCallback: (model: Model<string>, thinkingLevel: ModelThinkingLevel) => void;
	private onSelectAsDefaultCallback?: (model: Model<string>, thinkingLevel: ModelThinkingLevel) => void;
	private getInitialThinkingLevel: (model: Model<string>) => ModelThinkingLevel;
	// Picker-local drafts: browsing, refresh and cancellation never mutate the session.
	private thinkingLevels = new Map<string, ModelThinkingLevel>();
	...
	constructor(
		...
		onSelect: (model: Model<string>, thinkingLevel: ModelThinkingLevel) => void,
		onCancel: () => void,
		initialSearchInput?: string,
		onSelectAsDefault?: (model: Model<string>, thinkingLevel: ModelThinkingLevel) => void,
		defaultModel?: DefaultModelReference,
		getThinkingLevel: (model: Model<string>) => ModelThinkingLevel = () => "off",
	) {
		super();
		...
		this.getInitialThinkingLevel = getThinkingLevel;
	...
	private updateList(): void {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const name = item.model.name.trim() || item.id;
			const modelText = isSelected ? theme.fg("accent", name) : name;
			const idBadge = name === item.id ? "" : theme.fg("muted", ` · ${item.id}`);
			const providerBadge = theme.fg("muted", `[${item.provider}]`);
			const line = `${cursor}${currentMarker}${modelText}${idBadge} ${providerBadge}${defaultBadge}`;

			// Keep each model on one row, including long names and narrow terminals.
			// The highlighted model's full identity remains available below the list.
		...
		if (selected) {
			...
			this.listContainer.addChild(new Text(theme.fg("muted", `  Model Name: ${model.name}`), 0, 0));
			this.listContainer.addChild(new Text(theme.fg("muted", `  Model ID: ${model.provider}/${model.id}`), 0, 0));
			const levels = getSupportedThinkingLevels(model);
			const currentLevel = this.getThinkingLevel(model);
			const choices = levels
				.map((level) => (level === currentLevel ? theme.fg("accent", `[${level}]`) : theme.fg("muted", level)))
				.join(" ");
			const hint =
				levels.length > 1
					? `${keyDisplayText("app.thinking.cycle")} cycles`
					: model.reasoning
						? "fixed by model/provider"
						: "not supported";
			this.listContainer.addChild(new Text(`  Thinking: ${choices} ${theme.fg("dim", `· ${hint}`)}`, 0, 0));
		}
	...
	handleInput(keyData: string): void {
		...
		if (kb.matches(keyData, "app.thinking.cycle")) {
			const model = this.filteredModels[this.selectedIndex]?.model;
			if (model) {
				const levels = getSupportedThinkingLevels(model);
				if (levels.length > 1) {
					const index = levels.indexOf(this.getThinkingLevel(model));
					this.thinkingLevels.set(`${model.provider}\0${model.id}`, levels[(index + 1) % levels.length]);
					this.updateList();
				}
			}
			return;
		...
		// Select and save as default
			...
			if (selectedModel) {
				this.dispose();
				this.onSelectAsDefaultCallback(selectedModel.model, this.getThinkingLevel(selectedModel.model));
			}
		}
	...
	private getThinkingLevel(model: Model<string>): ModelThinkingLevel {
		const requested =
			this.thinkingLevels.get(`${model.provider}\0${model.id}`) ?? this.getInitialThinkingLevel(model);
		// A refresh may remove a previously drafted level. Never submit stale capabilities.
		return clampThinkingLevel(model, requested);
	}

	private handleSelect(model: Model<any>): void {
		this.dispose();
		this.onSelectCallback(model, this.getThinkingLevel(model));
	}
