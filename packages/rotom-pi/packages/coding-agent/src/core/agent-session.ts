...
export class AgentSession {
	 ...
	 * An explicit thinkingLevel applies before model_select, without saving a thinking default.
	 * @throws Error if no auth is configured for the model
	 */
	async setModel(
		model: Model<any>,
		options: ModelMutationOptions & { thinkingLevel?: ThinkingLevel } = {},
	): Promise<void> {
		...
		const thinkingLevel = this._getThinkingLevelForModelSwitch(model, options.thinkingLevel);
	...
	/** Preview a picker selection without mutating model, session history or defaults. */
	getThinkingLevelForModel(model: Model<string>): ThinkingLevel {
		const scopedLevel = this._scopedModels.find((scoped) => modelsAreEqual(scoped.model, model))?.thinkingLevel;
		const requested = modelsAreEqual(this.model, model)
			? this.thinkingLevel
			: this._getThinkingLevelForModelSwitch(model, scopedLevel);
