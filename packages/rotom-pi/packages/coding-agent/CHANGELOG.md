...
- Added thinking-level selection inside `/model` using the configured thinking-cycle shortcut. Drafts apply on selection and are discarded on cancel; fixed model capabilities remain enforced.

### Changed

- Show display names first in `/model`, with IDs and providers as secondary labels. Long names stay on one list row, with full identity in the details below.
