...
| `name` | No | `id` | Human-readable model label. Used for matching (`--model` patterns) and shown first in the `/model` picker. |
...
Current behavior:
- `/model` shows `name` first, with `id` and provider as secondary labels; a blank name falls back to `id`.
- `--list-models` and the interactive footer still use model `id`. Display names never replace protocol or saved-session IDs.
- Shift+Tab in `/model` drafts a supported thinking level for the highlighted model. Enter applies it with the model; Escape discards the draft. Fixed levels are marked explicitly.
...
- Overriding `name` changes model matching, the primary `/model` label and detail text; the footer and protocol identity continue to use model `id`.
