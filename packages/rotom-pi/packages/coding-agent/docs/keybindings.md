...
| `app.thinking.cycle` | `shift+tab` | Cycle thinking level; inside `/model`, draft the highlighted model's supported level until selection |
| `app.thinking.save` | `ctrl+s` | Save current thinking level to settings |
| `app.thinking.toggle` | `ctrl+t` | Collapse or expand thinking blocks |

Inside `/model`, Tab still switches all/scoped lists. Thinking drafts remain local to the picker until Enter or `app.models.save`; Escape discards them. Saving a model default does not change the global thinking default. Fixed-level and non-reasoning models do not expose extra levels.
