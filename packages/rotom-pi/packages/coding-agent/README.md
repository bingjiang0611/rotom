...
For each built-in provider, pi maintains a list of tool-capable models. Configured provider catalogs refresh automatically; run `pi update --models` to force an immediate refresh. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L). The picker shows model names first, with IDs and providers as secondary labels. Press Shift+Tab to cycle the highlighted model's supported thinking levels, then Enter to apply the model and level to this session. Escape discards the draft. Models with a fixed level are marked explicitly. Press Ctrl+S to apply the selection and save only the model as the startup default; use `/thinking` to save a thinking default.
...
| `/model` | Select by name or ID; Shift+Tab drafts thinking level, Enter applies, Ctrl+S saves the model default |
| `/thinking` | Switch thinking level; Ctrl+S in the picker saves the startup default |
| `/scoped-models` | Enable/disable models for Ctrl+P cycling; uses the same name-first labels as `/model` |
