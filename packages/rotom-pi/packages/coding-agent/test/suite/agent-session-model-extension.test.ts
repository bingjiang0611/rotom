...
describe("AgentSession model and extension characterization", () => {
	...
	it("applies a picker thinking level before model_select without persisting it as a default", async () => {
		const observed: ThinkingLevel[] = [];
		const harness = await createHarness({
			models: [
			...
			extensionFactories: [
				(pi) => {
					pi.on("model_select", () => {
						observed.push(pi.getThinkingLevel());
					});
				},
			],
		});
		harnesses.push(harness);
		await harness.session.setModel(harness.getModel("two")!, { persist: true, thinkingLevel: "high" });
		expect(observed).toEqual(["high"]);
		expect(harness.session.thinkingLevel).toBe("high");
		expect(harness.settingsManager.getDefaultModel()).toBe("two");
		expect(harness.settingsManager.getDefaultThinkingLevel()).toBe("medium");
	});

	it("does not apply a picker thinking level when model authentication fails", async () => {
		const harness = await createHarness({
			models: [
				{ id: "one", reasoning: true },
				{ id: "two", reasoning: true },
		...
		await expect(
			harness.session.setModel(harness.getModel("two")!, { thinkingLevel: "high", persist: true }),
		).rejects.toThrow("No API key");
		expect(harness.session.model?.id).toBe("one");
	...
	it("previews picker thinking without mutating history or defaults", async () => {
		const harness = await createHarness({
			models: [
				{ id: "one", reasoning: true },
				{ id: "two", reasoning: true },
		...
		harnesses.push(harness);
		const one = harness.getModel("one")!,
			two = harness.getModel("two")!,
			plain = harness.getModel("plain")!;
		harness.session.setThinkingLevel("high");
		const count = harness.sessionManager.getEntries().length;
		expect(harness.session.getThinkingLevelForModel(one)).toBe("high");
		expect(harness.session.getThinkingLevelForModel(two)).toBe("medium");
		harness.settingsManager.setModelThinkingLevel(two.provider, two.id, "low");
		expect(harness.session.getThinkingLevelForModel(two)).toBe("low");
		harness.session.setScopedModels([{ model: two, thinkingLevel: "high" }]);
		expect(harness.session.getThinkingLevelForModel(two)).toBe("high");
		expect(harness.session.getThinkingLevelForModel(plain)).toBe("off");
		expect(harness.session.model?.id).toBe("one");
		expect(harness.session.thinkingLevel).toBe("high");
		expect(harness.sessionManager.getEntries()).toHaveLength(count);
		expect(harness.settingsManager.getDefaultThinkingLevel()).toBe("medium");
