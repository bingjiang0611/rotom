...
appear (or disappear with `until: "absent"`) before reporting success. It records
...
One action is represented by an array of length one. A multi-action transaction is appropriate only when no intermediate observation is needed. The runtime validates one base state and acquires one resource lane. Native batches capture a pre-transaction root baseline; steps are executed and verified in order, stopping on the first failed or invalidated step. Partial results include `stoppedAt`, so callers know the exact checked boundary and must re-observe before continuing. Strict-headless calls use native transactional batching; fallback-capable calls may execute checked steps sequentially while retaining dependent focus. The bridge produces one final observation. This is not a mechanism for parallel actions within one UI resource.
