...
// rotom fork enhancement (see UPSTREAM.md, "speculative foreground retry"):
//
// The speculative foreground re-dispatch re-posts keyboard events for
// typeText/keypress that were ALREADY delivered once in the background attempt.
// A "didnt" outcome here only means "no observed value change" (evidence.value
// did not change within the short post-input read); with async editors or
// delayed AX updates the first input may still have taken effect, so an
// automatic second dispatch risks duplicated input. rotom's contract is to
// perform an external write once and return didnt/unknown rather than replay it,
// letting the model re-observe and decide. This speculative retry is therefore
// disabled by default; the explicit `foreground_required` path in helperAct
// (where the native layer proved the write was NOT applied) is unaffected.
//
// Set ROTOM_CU_SPECULATIVE_FOREGROUND_RETRY=1 to restore the upstream behaviour.
export function canRetryInForeground(
	...
	const optedIn = /^(?:1|true|yes)$/iu.test((env.ROTOM_CU_SPECULATIVE_FOREGROUND_RETRY ?? "").trim());
