...
async function appendHarnessRunReport(test: TestCase): Promise<void> {
	const artifactDirectory = process.env.ROTOM_EVAL_ARTIFACT_DIR?.trim();
	...
	const metadata = Object.fromEntries(
		Object.entries(run.artifacts ?? {}).filter(
			([name]) => name !== "runId" && name !== PI_SESSION_SNAPSHOT_ARTIFACT && name !== PI_TRACE_SNAPSHOT_ARTIFACT,
		),
	);
	const record = {
		schemaVersion: 1,
	...
	await ensurePrivateEvalArtifactDirectory(artifactDirectory);
	await writePrivateEvalArtifactFile(join(artifactDirectory, "runs.jsonl"), `${JSON.stringify(record)}\n`, "append");
}
...
export default class EvalHarnessReporter implements Reporter {
	...
	async onTestRunEnd(
		...
		if (artifactDirectory) {
			await ensurePrivateEvalArtifactDirectory(artifactDirectory);
			await writePrivateEvalArtifactFile(join(artifactDirectory, "comparisons.json"), `${JSON.stringify(report, null, 2)}\n`);
		}
	}
}
