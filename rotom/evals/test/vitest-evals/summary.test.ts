...
function observation(
	...
	metrics: Pick<HarnessObservation, "totalTokens" | "toolCalls" | "totalMs" | "estimatedCostUsd"> = {},
...
describe("summarizeHarnessComparisons", () => {
	it("computes paired correctness lift separately from efficiency deltas", () => {
		const report = summarizeHarnessComparisons([
			...
			observation("without-tools", "inspect", "passed", { totalTokens: 200, toolCalls: 2 }),
			observation("with-tools", "inspect", "passed", { totalTokens: 180, toolCalls: 2 }),
		]);
	...
	it("reports missing observations without coercing them to failures or zero telemetry", () => {
		...
		expect(comparison?.toolCalls).toEqual({
			totalPairs: 2,
			eligiblePairs: 0,
			baselineMean: null,
			candidateMean: null,
	...
	it("formats lift and telemetry availability for the terminal report", () => {
		...
		expect(formatted).toContain("Tool calls  unavailable");
