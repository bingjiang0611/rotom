...
async function traceSessionMetadata(root, path) {
	const sessionPath = sessionPathForTrace(path);
	if (!containedBy(root, sessionPath)) return { exists: false, sizeBytes: null };
	try {
		const metadata = await lstat(sessionPath);
		return metadata.isFile() && !metadata.isSymbolicLink()
			? { exists: true, sizeBytes: metadata.size }
			: { exists: false, sizeBytes: null };
	} catch { return { exists: false, sizeBytes: null }; }
}
...
export function summarizeTrace(root, path, loaded, now = Date.now()) {
	...
	return {
		...
		sessionSizeBytes: Number.isSafeInteger(loaded.sessionSizeBytes) ? loaded.sessionSizeBytes : null,
...
export async function scanTraceFiles(root) {
	...
	for (const path of collected.paths) {
		...
		try {
			...
			const session = await traceSessionMetadata(absoluteRoot, path);
			loaded.orphan = !session.exists;
			loaded.sessionSizeBytes = session.sizeBytes;
		...
		try {
			...
			try {
				...
				const detail = {
					...
					sessionSizeBytes: trace.summary.sessionSizeBytes,
				};
